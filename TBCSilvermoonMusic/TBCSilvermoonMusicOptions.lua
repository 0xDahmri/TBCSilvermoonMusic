...
frame:SetScript("OnEvent", function(self, event, addonName)
    ...
    if SilvermoonMusicDB.introOnEnter == nil then SilvermoonMusicDB.introOnEnter = false end
    ...
    -- Checkbox: intro on enter
    do
        local setting = Settings.RegisterAddOnSetting(
            category,
            "SilvermoonMusic_IntroOnEnter",
            "introOnEnter",
            SilvermoonMusicDB,
            Settings.VarType.Boolean,
            "Play Intro on Zone Entry",
            false
        )
        Settings.CreateCheckbox(
            category, setting,
            "Always play the zone-entry fanfare when entering Silvermoon, then continue with your track selection."
        )
    end
